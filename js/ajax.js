...
var products_json = [
...

    {
        ...
        "brand": "Paco Rabanne",
        "type": "Eau de Parfum",
        "image": "https://cdn.flaconi.de/media/catalog/215x/p/a/paco-rabanne-olympea-eau-de-parfum-50-ml.jpg",
        //"image": "https://cdn.flaconi.de/media/catalog/215x/g/i/giorgio-armani-si-eau-de-parfum-50ml_7.jpg",
        "price": 4395,
        "price": 3780,
        "size": "30ML",
...
var paginationObject =   {
    total: null, pages: [], itemPerPage: 5, currentPage: 1, numPages:1,

    set: function(total,itemPerPage){
    ...
    setPages: function(){
        this.pages=[];
        for(i = 1;i <= this.numPages;i++) this.pages.push(i);//console.log(this.pages);
    },
    prevPage : function() {
...
app.controller('customersCtrl', function($scope, $http) {
    ...
    $scope.AllProducts = products_json;
    $scope.products = products_json;
    $scope.pagination= paginationObject.set($scope.products.length,items_per_page);
    $scope.filterApply = function() {   };
    $scope.$watch('filteredProducts', function() {
        $scope.pagination= paginationObject.set($scope.filteredProducts,items_per_page);
        $scope.pagination.setPageId(1);
    });
    $scope.default_image = "images/default.jpeg";
    //sort
    $scope.OrderByName = function() {//on:order_name
        var order_col = $scope.order_col.split('#');
         $scope.on = order_col[0];
         $scope.order_reverse = (order_col[1] == 'asc') ? false : true;
    }

    $scope.pagination.setPageId(1);
    $scope.changePage = function() {
        $scope.pagination.setPageId($scope.selectedPage);
    };
    ...
    $scope.ChangeFilter = function () {
        ...
        AllFilteredProducts = $scope.AllProducts;
        if($scope.filter_PName !== undefined && $scope.filter_PName.trim()!= "")
        {
            for (var i = 0; i < AllFilteredProducts.length; i++) {
                if (AllFilteredProducts[i].name.toLowerCase().includes($scope.filter_PName.toLowerCase()))
                {
                    newArray.push(AllFilteredProducts[i]);
                }
            }
            AllFilteredProducts = newArray;
            newArray = [];
        }

        if($scope.filter_PBrand !== undefined && $scope.filter_PBrand.trim()!= "")
        {
            for (var i = 0; i < AllFilteredProducts.length; i++) {
                if (AllFilteredProducts[i].brand.toLowerCase().includes($scope.filter_PBrand.toLowerCase()))
                {
                    newArray.push(AllFilteredProducts[i]);
                }
            }
        ...
        if($scope.filter_FromPrice !== undefined && $scope.filter_FromPrice.trim()!= "")
        {
            for (var i = 0; i < AllFilteredProducts.length; i++) {
                if (AllFilteredProducts[i].price >= $scope.filter_FromPrice.toLowerCase())
                {
                    newArray.push(AllFilteredProducts[i]);
                }
            }
        ...
        if($scope.filter_ToPrice !== undefined && $scope.filter_ToPrice.trim()!= "")
        {
            for (var i = 0; i < AllFilteredProducts.length; i++) {
                if (AllFilteredProducts[i].price <= $scope.filter_ToPrice.toLowerCase())
                {
                    newArray.push(AllFilteredProducts[i]);
                }
            }
        ...
        $scope.products = AllFilteredProducts;
        ...
        $scope.selectedPage = $scope.pagination.pages[0]; 
        if($('#brand').is(':focus')) $scope.groupBy( 'brand' );     
    };
    ...
    $scope.groupBy = function( attribute ) {
        ...
        $('#filter_container').css({top: position.top + 30, left: position.left}).width($('#'+attribute).width()-2);
        $('#filter_container').show();
         $scope.groups = []; //The temporary array to keep the values
         for ( var i = 0 ; i < $scope.products.length ; i++ ) {
            var item = $scope.products[ i ];
             if($scope.groups.indexOf(item[ attribute ]) === -1) { //Check if the values is not already in the temp array
                 $scope.groups.push(item[ attribute ]); //Push it to array
             }
         };
         $scope.groups.sort();
    };
...
$(document).ready(function() {
    ...
    $('#filter_container input[type=button]').on('click',function(){
